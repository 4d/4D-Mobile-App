...
var $Obj_project; $Obj_parent; $Obj_children; $Obj_uuid; $Obj_intermediate; $Obj_buffer : Object

var $File_buffer; $Txt_path; $Txt_parentpath; $Txt_filename; $Txt_parentuuid; $Txt_intermediate; $Txt_currentPath : Text

var $Col_intermediate : Collection
...
Case of 
		...
		For each ($Obj_children; $Col_intermediate)
			...
			If (Value type:C1509($Obj_children.children)=Is collection:K8:32)
				...
				If (Feature.with("buildWithCmd"))
					$Obj_buffer:=FolderFrom(String:C10($Obj_children.source))
				Else 
					$Obj_buffer:=Path to object:C1547(String:C10($Obj_children.source))  // not server l compliant
				End if 
				...
				Case of 
					...
					: (String:C10($Obj_buffer.extension)=".xcassets")
						...
						If (($Obj_buffer.name="Assets") && ((Feature.with("buildWithCmd") ? $Obj_buffer.parent.name : Path to object:C1547($Obj_buffer.parentFolder).name)="Resources"))
							
							// Already inserted in default project
							
						Else 
							
							ob_warning_add($Obj_out; "Asset '"+String:C10($Obj_children.source)+"' will not be injected automatically in project. (Resources/Assets.xcassets are managed)")
							
						End if 
						
						//----------------------------------------
					: (String:C10($Obj_buffer.name)=SHARED.thirdParty)
	...
	: (Value type:C1509($Obj_in.path)=Is text:K8:3)
		...
		If (Position:C15("/"+SHARED.thirdParty+"/"; $File_buffer)=0)  // ignore thirdparty file, will be injected by sdk"
