...
If ($Lon_Parameters<4)
	...
	_o_rgx_Lon_Error:=-50  // Parameter error
...
Else 
	...
	If (Length:C16($Txt_groups)>0)
		...
		_o_Rgx_SplitText("\\s"; $Txt_groups; ->$tTxt_Groups; (0 ?+ 1) ?+ 11)
	...
	_o_rgx_Lon_Error:=-50*Num:C11(($Lon_Groups>1) & Not:C34($Boo_Array2D))  // Parameter error
	
	$Txt_Error_Method:=Method called on error:C704
	ON ERR CALL:C155("_o_rgx_NO_ERROR")
	
	$Lon_Start:=1
	
	If (_o_rgx_Lon_Error=0)
		
		If ($Lon_Parameters>=5)
			
			$Lon_options:=$5  // (optional) Regular expression flags
			$Txt_pattern:=_o_rgx_Options($Lon_options)+$Txt_pattern
		...
		If (_o_rgx_Lon_Error=0)
			...
			_o_rgx_Lon_Error:=-Num:C11(Size of array:C274($Ptr_resultArray->)=0)
