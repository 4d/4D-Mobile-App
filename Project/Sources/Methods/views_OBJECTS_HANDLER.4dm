			...
			: ($Obj_form.form.event=On Clicked:K2:4)
				...
				Case of 
					...
					: ($Txt_tableNumber#$Obj_context.tableNum())\
						 ...
						 | (Test path name:C476(_o_Pathname ("listforms")+$Txt_template+Folder separator:K24:12)#Is a folder:K24:2)
	...
	: ($Obj_form.form.currentWidget=$Obj_form.fieldList.name)
		...
		Case of 
			...
			: ($Obj_form.form.event=On Begin Drag Over:K2:44)
				...
				$p:=svg .embedPicture(ui.fieldIcons[$Obj_current.fieldType];2;2).textArea($Obj_current.path+" ";20;2).attribute("font-size";13).get("picture")
