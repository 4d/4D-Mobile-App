		...
		If ($Boo_OK)  // & (Bool(featuresFlags._103411))
			...
			Else 
				...
				If ($o.success)
					...
					$Boo_OK:=($o.dataClasses.extract("name").indexOf(String:C10(commonValues.deletedRecordsTable.name))#-1)
					...
					If ($Boo_OK)
						...
						For each ($t;$c) While ($Boo_OK)
							...
							If ($Boo_OK)
								...
								$Boo_OK:=($o.dataClasses[$l].attributes.extract("name").indexOf(String:C10(commonValues.stampField.name))#-1)
