...
Case of 
	...
	: ($Obj_in.action="mergeObjects")  // from project 4d file, return the list of capabilities
		...
		If (Value type:C1509($Obj_in.value)=Is collection:K8:32)
			...
			$Obj_out.value:=$Obj_in.value.reduce(Formula:C1597(col_formula).source; New object:C1471; "$1.accumulator:=ob_deepMerge ($1.accumulator;$1.value)")
	...
	: ($Obj_in.action="read")  // read files from project target
		...
		Case of 
			...
			: ($Obj_types[String:C10($Obj_in.type)]#Null:C1517)  // want only a specific types
				...
				$Obj_out[$Obj_in.type]:=plistconvert(New object:C1471(\
					"action"; "object"; \
			...
			Else   // read all
				...
				For each ($Txt_fileType; $Obj_types)
					...
					$Obj_out[$Txt_fileType]:=plistconvert(New object:C1471(\
						"action"; "object"; \
	...
	: ($Obj_in.action="natify")  // transform simple key to real key
		...
		If (Value type:C1509($Obj_in.urlSchemes)=Is collection:K8:32)
